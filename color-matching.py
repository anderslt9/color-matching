...
class Image_Process:
  def __init__(self, img_path, reshape_size=None):
  ...
  def reshape_img(self, reshape_size):
    ...
    if type(reshape_size) != tuple: 
      ...
      assert type(reshape_size) == float or type(reshape_size) == int
      self.img = transform.resize(self.img, (int(self.img.shape[0] * reshape_size), int(self.img.shape[1] * reshape_size), 3))
    ...
    if reshape_size[0] > 0 and reshape_size[1] > 0: 
      self.img = transform.resize(self.img, (*reshape_size,3)) 
      self.img_size = self.img.shape[:2]
      return
    ...
    if reshape_size[0] > 0: 
      ...
      self.img = transform.resize(self.img, (reshape_size[0], int(self.img.shape[1] * ratio), 3))
    else:
      ratio = reshape_size[1] / self.img.shape[1]
      self.img = transform.resize(self.img, (int(self.img.shape[0] * ratio), self.img.shape[1], 3))
